...
def keybindings_screen(lives=4):
    keybindText1 = FONT.render("Press M to mute/unmute or just click the symbol.", 1, "orange" if lives <= 3 else "white")
    keybindText2 = FONT.render("Use A & D keys to move left & right.", 1, "orange" if lives <= 3 else "white")
    keybindText3 = FONT.render("Press P or click the symbol to pause the game ", 1, "orange" if lives <= 3 else "white")
    keybindText4 = FONT.render("Press I or K to bring up this screen.", 1, "orange" if lives <= 3 else "white")
    WINDOW.blit(welcome_screen_background if lives <= 3 else game_background_blurred, (0, 0))
    ...
    resumeText = FONT_SMALL.render("Click any key to " + ("continue!" if lives <= 3 else "resume!"), 1,
                                   "orange" if lives <= 3 else "white")
